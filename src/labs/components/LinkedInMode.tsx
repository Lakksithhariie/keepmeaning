...
export const LinkedInMode = () => {
  ...
  const MISTRAL_API_KEY = 'DcAIy22l4lOMxdUBen4cJVI3mWFTkAh3';
  ...
  const syncMemoryWithCloud = async (email: string) => {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        ...
        try { parsedArchive = userRow.history ? JSON.parse(userRow.history) : []; } catch (e) { parsedArchive = []; }
  ...
  const loadSpecificRitual = (ritual: RitualSession) => {
    ...
    setCurrentHistory(ritual.chatHistory || []);
    ...
    try {
      await fetch(`https://api.baserow.io/api/database/rows/table/${TABLE_ID}/${baserowRowId}/?user_field_names=true`, {
        ...
        body: JSON.stringify({
          ...
          'history': JSON.stringify(updatedArchive),
          'usage_count': updatedArchive.reduce((acc, curr) => acc + (curr.chatHistory.length / 2), 0)
        })
      });
    } catch (error) { console.error(error); } finally { setIsSyncing(false); }
  };
  ...
  const handleGenerate = async (isRefining = false) => {
    ...
    try {
      ...
      if (isRefining) { currentHistory.forEach(msg => draftMessages.push(msg)); draftMessages.push({ role: 'user', content: `REFINE (STAY ON LENS: ${lens?.name}): ${refinementInput}` }); } 
  ...
  const handleLogout = () => {
    localStorage.removeItem('km_linkedin_email');
    localStorage.removeItem('km_temp_draft');
    localStorage.removeItem('km_temp_anchor');
    setUserEmail('');
    setBaserowRowId(null);
    setRitualsArchive([]);
    setActiveRitualId(null);
    setDraft('');
    setAnchor('');
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="bg-white border border-[#E5E5E3] rounded-[40px] shadow-[0_40px_120px_rgba(0,0,0,0.1)] max-w-xl w-full overflow-hidden">
              ...
              <div className="p-12 md:p-16 space-y-10 text-center">
                ...
                <div className="space-y-4 text-left">
                  ...
                  <button onClick={handleAuth} disabled={!emailInput.includes('@')} className="w-full bg-black text-white py-5 rounded-2xl font-bold text-sm uppercase tracking-widest flex items-center justify-center gap-4 hover:bg-km-red transition-all shadow-xl disabled:opacity-20">Initialize Cloud Memory <ArrowRight size={18} /></button>
                </div>
          ...
          {isSidebarOpen && (
            ...
            >
              ...
              <div className="p-6 border-b border-[#F0F0EE] bg-white relative">
                ...
                <button onClick={() => navigate('/canvas')} className="text-[#CBCBC9] hover:text-km-red flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest transition-all mb-6"><ArrowLeft size={12} /> Back to Ghostwriter</button>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col text-left">
                    ...
                    <div className="flex items-center gap-2 mt-1"><span className="text-[9px] font-serif italic text-[#CBCBC9] tracking-widest uppercase">LinkedIn Studio</span><div className="w-1 h-1 rounded-full bg-[#ECECEC]" /><button onClick={handleLogout} className="text-[9px] font-bold text-km-red uppercase tracking-widest hover:underline">Switch</button></div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8 text-left">
                <AnimatePresence mode="wait">
                  {activeTab === 'pilot' ? (
                    <motion.div key="pilot-ui" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                    ...
                    <motion.div key="archive-ui" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
